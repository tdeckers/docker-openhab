...
RUN wget --no-check-certificate --no-cookies --header "Cookie: oraclelicense=accept-securebackup-cookie" -O /tmp/jdk-7u75-linux-x64.tar.gz http://download.oracle.com/otn-pub/java/jdk/7u75-b13/jdk-7u75-linux-x64.tar.gz
RUN tar -zxC /opt -f /tmp/jdk-7u75-linux-x64.tar.gz
RUN ln -s /opt/jdk1.7.0_75 /opt/jdk7
